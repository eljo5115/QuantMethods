binscatter <- function(formula, key_var, data, bins=20, partial=FALSE, xlab=NULL,
                       ylab=NULL, title = ""){
  ...
  xlab <- ifelse(is.null((xlab)), names(df)[2], xlab)
  ylab <- ifelse(is.null((ylab)), names(df)[1], ylab)
  df_bin <- aggregate(df,by=list(cut(as.matrix(df[,2]),bins)), mean)
  
  ggplot(data=df, aes(x=df[,2], y=df[,1])) +
    geom_ribbon(aes(ymin=lower_ci, ymax=upper_ci),alpha=0.5, fill="bisque2") +
    geom_point(data=df_bin, aes(x=df_bin[,3], y=df_bin[,2]), color="orangered4", size=1.5) +
    geom_point(alpha=0.4, shape = 18) + 
    geom_segment(aes(x = min_x, y = min_y, xend = max_x, yend = max_y),
                 color="royalblue4", size=0.7) +
